...
use Input, Session, Redirect, Validator, Response, Mail, Log;
use Omnipay;
    ...
    */
    ...
    public function addDesign()
    {
        if($saved = MyCart::hasSavedDesign())
        {
            ...
            if($product->design->require_card_design == YES)
            {
                //validate fields
                $rules = [
                    'card-brief-email'          => 'required|email',
    ...
    public function purchase()
    {
        ...
        else if($payment == PAYMENT_CREDIT)
        {
            ...
            Omnipay::setGateWay('paypalpro');
            $response = Omnipay::purchase($params)
                ->setItems($items)
                ->send();  
            if ($response->isSuccessful())
            {
                $cardTypes = get_creditcard_types();    
                $cardType = $cardTypes[$cardInfo['type']];
                
                $transactionId = $response->getTransactionReference();
                if(MyCart::placeOrder($currency, $transactionId, $cardType . '-' . str_replace(' ', '', $cardInfo['number']), ORDER_NEW))
                {
        ...
        if(Input::has('payment-method'))
        {
            ...
            if($paymentMethod == PAYMENT_CREDIT || $paymentMethod = PAYMENT_PAYPAL)
            {
                ...
                else
                {
                    ...
                    $idDefaultCard  = Input::get('default-card');
                    $requireSaveCard= Input::get('remember-card');
                    $cardTypes = get_creditcard_types();
                    ...
                    else
                    {
                        $paymethod = [
                            ...
                            'card' => [
                                ...
                                'year'  => $expYear,
                                'cvv'   => $cvc,
                                'default' => $idDefaultCard,
                                'save'  => $requireSaveCard
                            ]
                        ];
                        MyCart::savePaymentMethod((object)$paymethod);
