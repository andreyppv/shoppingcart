...
class SettingController extends FrontController {
    ...
    public function postAccount()
    {
        ...
        $subscribe = Input::get('newsletter') ? 1 : 0;
        ...
        //subscribe to Aweber
        if($this->current_customer->is_subscribed != $subscribe)
        {
            $aweber = new AWeber(); 
            if($subscribe == 1)
            {
                $aweber->addSubscriber(Input::get('email'));
            }
            else
            {
                $aweber->removeSubscriber(Input::get('email'));
            }
        }
        //end subscribe
        
        $this->current_customer->is_subscribed = $subscribe;
