...
class MyCart
{
    ...
    */
    ...
    public static function savePayload()
    {
        ...
        $info['currency'] = self::getCurrencyFromAddress();
        $info['coupon'] = self::getCoupon();
    ...
    public static function saveShippingAddress($address)
    {
        ...
        //save currencyRate
        if(self::getCurrencyFromAddress() == 'CAD')
        {
            $cart = self::get();
            $cart->currency_rate = Setting::getCurrencyRate();
            $cart->save();
        }        
    }
    ...
    public static function placeOrder($currency, $transactionId='', $payerId='', $orderStatus=ORDER_PENDING)
    {
        ...
        if($orderSaved)
        {
            ...
            foreach($cart->items as $item)
            {
                ...
                $orderItem->unit_price  = $item->unit_price * $cart->currency_rate;
        ...
        {
            self::$current_customer->saveShippingAddress(
    ...
    public static function addDesign($productId, $referenceOrdeId=0)
    {
        ...
        //add design info   
        self::_addDesignInfo($cart->id, $item->id, $product->design->require_logo_design, $product->design->require_card_design);
    ...
    private static function _addDesignInfo($cartId, $itemId, $requireLogoDesign, $requireCardDesign)
    {
        if($requireLogoDesign == NO && $requireCardDesign == NO) return;
