    ...
    <figure itemprop="associatedMedia" itemscope itemtype="http://schema.org/ImageObject" data-title="{{ $product->name }}" data-image="{{ $img->url() }}">
        ...
        <figcaption itemprop="caption description">
            <div class="share-icons-wrap">
                <button class="pswp__button" style="display: inline; float: none; width: 50px; height: 50px;margin: 0;background: transparent;"><a href="<?php echo sprintf($pinUrl, urlencode($product->getUrl() . "#&gid=1&pid=".($i+1)), urlencode(asset($img->image)), urlencode($product->name)); ?>" class="share-btn"><img alt="share-icon" src="{{ asset('frontend/img/soc-pin-icon(grey).svg') }}"></a></button>
                <button class="pswp__button" style="display: inline; float: none; width: 50px; height: 50px;margin: 0;background: transparent;"><a href="<?php echo sprintf($fbUrl, urlencode($img->url())); ?>" class="share-btn"><img alt="share-icon" src="{{ asset('frontend/img/soc-fac-icon(grey).svg') }}"></a></button>
                <button class="pswp__button pswp__button--show btn-show-bubbles" title="Show / Hide Bubbles" style="color:white; background:none; width:auto;">View Feature Info</button>
            </div>
