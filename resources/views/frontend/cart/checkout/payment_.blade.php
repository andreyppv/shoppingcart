...
{!! Form::open(array('id' => 'payment-form', 'method' => 'post')) !!}
    ...
    <div class="col-md-6">
        ...
        <div id="credit-card-info" class="credit-card-info">
            ...
            @if(!$current_customer->creditCards->isEmpty())
            <div class="credit-card-row">
                <div class="left-bl">Saved Cards</div>
                <div class="right-bl">
                    <select>
                        @foreach($current_customer->creditCards as $card)
                        <option value="{{ $card->cc_id }}">{{ $card->cc_name }} ( {{ $card->type }}:{{ $card->cc_number }} )</option>
                        @endforeach
            ...
            <div id="new-card-box" style="<?php if($current_customer->creditCards->isEmpty()) { echo "display: none;"; } ?>">
                <div class="credit-card-row">
                ...
                <div class="check-bl-wrap">
                    <div class="check-bl">
                        <div class="checkbox check-success">
                            {!! Form::checkbox('default-card', 1, false, array('id' => 'save_as_default_card', 'readonly')) !!}
                            <label for="save_as_default_card">Set this card as default payment method</label>
                        </div>
                    </div>
                    <div class="check-bl">
                        <div class="checkbox check-success">
                            {!! Form::checkbox('remember-card', 1, false, array('id' => 'save_card', 'readonly')) !!}
                            <label for="save_card">Remember this card</label>
                        </div>
                    </div>
                </div> 
                @endif
