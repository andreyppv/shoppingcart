...
{!! Form::open(array('url' => URL::route("admin.design.category.store", $row->id), 'method' => 'post', 'class' => 'form-horizontal', 'id' => 'aform', 'autocomplete' => 'off', 'files' => true)) !!}
    ...
    <div class="block">
        <div class="block-content">
            ...
            <div class="form-group {{ $errors->has('design-require-logo') ? 'has-error' : '' }}">
                <label class="col-sm-2 control-label" for="slide">Require Card Design<span class="red">*</span></label>
                <div class="col-sm-9">
                    {!! Form::select('design-require-card', $ynDropdown, $row->require_card_design, array('class' => 'form-control autowidth', 'id' => 'design-require-card')) !!}
                    <div class="help-block">{{ $errors->first('design-require-card', ':message') }}</div>
                </div>
            </div>
