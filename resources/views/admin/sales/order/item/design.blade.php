...
<tbody>
    ...
    <tr>
        <td class="text-middle" rowspan="4" colspan="2"><img src="{{ resize_image( $item->image, 320, 320, 'fit-x' ) }}" alt=""></td>
        <td rowspan="4">
            <h5>{{ $item->name }}</h5>
    ...
    <tr>
        ...
        <td class="text-middle">
            @if($current_user->has_permission('sales.order.manage'))
                @if($item->job_status != JOB_UNASSIGNED)
                <input type="submit" class="btn btn-info" name="btn-job-complete" value="Submit"/>
                @endif
            @endif
        </td>
    </tr>
